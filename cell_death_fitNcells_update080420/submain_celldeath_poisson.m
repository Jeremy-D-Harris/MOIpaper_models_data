function results = submain_celldeath_poisson(data,FACs_data_18hpi_vector,alpha_init,Ncells_init,cell_line)

% updated on 2/14/20
which_distribution = 1; % Poisson
...
% total number of data points across all treatments and replicates
...
disp('Fitting cell death models under a Poisson distribution...')
...
disp(['time-independent, input-independent cell death rate: model ',num2str(model_num),' (Poisson)']); fprintf('\n');
...
var_log_alpha_m1 = get_var_poisson(log_alpha_timeindependent_inputindependent, moi_vector_3hpi, moi_vector, moi_vector_FACs_data,data.hpi, counts_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num);
alpha_m1 = exp(log_alpha_timeindependent_inputindependent');

CI_m1 = get_confidinterval(log_alpha_timeindependent_inputindependent',var_log_alpha_m1,z_score);
...
disp(['time-dependent, input-independent cell death rate: model ',num2str(model_num),' (Poisson)']); fprintf('\n');
...
disp(['time-independent, input-dependent cell death rate: model ',num2str(model_num),' (Poisson)']); fprintf('\n');

mu_m3 = 0.02;

% initialize minimization - per capita death rate
x0_timeindependent_inputdependent = log([alpha_init(4:5),mu_m3,Ncells_init]); % log(c): alpha(t,i) = c*i^eps

k_timeindependent_inputdependent = length(x0_timeindependent_inputdependent);

% minimize sum squared error to get estimates of log(c) and log(eps)
[log_alpha_timeindependent_inputdependent, LSE_percent_celldeath_timeindependent_inputdependent] = fminsearch(@(x)getfit_deathrate_dispersion(x,moi_vector_3hpi,moi_vector,moi_vector_FACs_data,counts_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timeindependent_inputdependent);

AIC_model3 = n_pts_allhpi_FACs*log(LSE_percent_celldeath_timeindependent_inputdependent)+2*k_timeindependent_inputdependent;

var_log_alpha_m3 = get_var_poisson(log_alpha_timeindependent_inputdependent, moi_vector_3hpi, moi_vector, moi_vector_FACs_data, data.hpi, counts_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num);
alpha_m3 = exp(log_alpha_timeindependent_inputdependent');

CI_m3 = get_confidinterval(log_alpha_timeindependent_inputdependent',var_log_alpha_m3,z_score);
...
disp(['time-dependent, input-dependent cell death rate: model ',num2str(model_num),' (Poisson)']); fprintf('\n');

% minimize sum squared error to get estimates of log(k), log(d), log(eps)
% initialize minimization - per capita death rate
x0_timedependent_inputdependent = log([alpha_init(6:8),mu_m4,Ncells_init]); % 'k','d','eps': alpha(t,i) = b4*k4*t^(k4-1)*i^eps

k_timedependent_inputdependent = length(x0_timedependent_inputdependent);

% minimize sum squared error to get estimates of log(k), log(d), log(eps)
[log_alpha_timedependent_inputdependent, LSE_percent_celldeath_timedependent_inputdependent] = fminsearch(@(x)getfit_deathrate_dispersion(x,moi_vector_3hpi,moi_vector,moi_vector_FACs_data,counts_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timedependent_inputdependent);

AIC_model4 = n_pts_allhpi_FACs*log(LSE_percent_celldeath_timedependent_inputdependent)+2*k_timedependent_inputdependent;

var_log_alpha_m4 = get_var_poisson(log_alpha_timedependent_inputdependent, moi_vector_3hpi, moi_vector, moi_vector_FACs_data, data.hpi, counts_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num);
alpha_m4 = exp(log_alpha_timedependent_inputdependent');

CI_m4 = get_confidinterval(log_alpha_timedependent_inputdependent',var_log_alpha_m4,z_score);
...
% figure 1 - model fits to percent cells remaining vs. Bulk MOI
% figure 2 - cell death rate vs. hpi
% figure 5 (top panel) - model fits to percent of alive cells that are infected vs. Bulk MOI
...
for k = 1:length(data.hpi)
    ...
    m1_cellsremaining = (alpha_m1(end)/100)*get_percentalive(alpha_m1,data.hpi(k),moi,1,1,which_distribution); % last two: which model, which distribution
    m2_cellsremaining = (alpha_m2(end)/100)*get_percentalive(alpha_m2,data.hpi(k),moi,1,2,which_distribution);
    m3_cellsremaining = (alpha_m3(end)/100)*get_percentalive(alpha_m3,data.hpi(k),moi,1,3,which_distribution);
    m4_cellsremaining = (alpha_m4(end)/100)*get_percentalive(alpha_m4,data.hpi(k),moi,1,4,which_distribution);
...
%     axis([0 10 0 100]);
    ...
    if k == 4 % 18 hpi time point
        ...
        m1_percentremaining_infected_poisson = get_percentaliveinfected(alpha_m1,data.hpi(k),moi,1,1,which_distribution);
        m2_percentremaining_infected_poisson = get_percentaliveinfected(alpha_m2,data.hpi(k),moi,1,2,which_distribution);
        m3_percentremaining_infected_poisson = get_percentaliveinfected(alpha_m3,data.hpi(k),moi,1,3,which_distribution);
        m4_percentremaining_infected_poisson = get_percentaliveinfected(alpha_m4,data.hpi(k),moi,1,4,which_distribution);
...
%         subplot(3,1,1);
        ...
        q(3) = plot(moi,m1_percentremaining_infected_poisson,'k--','LineWidth',1); hold on;
        q(4) = plot(moi,m2_percentremaining_infected_poisson,'k','LineWidth',1); hold on;
        q(5) = plot(moi,m3_percentremaining_infected_poisson,'k--','LineWidth',3); hold on;
        q(6) = plot(moi,m4_percentremaining_infected_poisson,'k','LineWidth',3); hold on;
        axis([0 10 0 100]);
        xlabel('Bulk MOI'); ylabel('Percent of alive cells that are infected');
        title([fig_title,': Poisson']);
        ax = gca;
        ax.FontSize = 10;
        ax.FontWeight = 'bold';
        ax.FontName = 'Arial';
...
figure(1);
sp = suptitle([fig_title,': Poisson']);
sp.FontSize = 12;
sp.FontWeight = 'Bold';


legend(r,{'time-independent, input-independent','time-dependent, input-independent','time-independent, input-dependent','time-dependent, input-dependent'},'Location','SouthWest');
...
results.percentcells_alive_infected = [m1_percentremaining_infected_poisson',m2_percentremaining_infected_poisson',m3_percentremaining_infected_poisson',m4_percentremaining_infected_poisson'];
