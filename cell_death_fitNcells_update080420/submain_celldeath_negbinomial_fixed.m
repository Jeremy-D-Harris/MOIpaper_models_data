function results = submain_celldeath_negbinomial_fixed(data,FACs_data_18hpi_vector,alpha_init,Ncells_init,r_dispersion_fixed,cell_line)
...
disp('Fitting cell death models under a negative binomial (dispersion parameter) to data...')
...
disp(['time-independent, input-independent cell death rate: model ',num2str(model_num),' (Negative binomial)']); fprintf('\n');

% constant death rate:
mu_m1 = 0.02;

% model 1: dispersion parameter:
dispersion_m1 = r_dispersion_fixed;

% initialize minimization - cell death rate
x0_timeindependent_inputindependent = log([alpha_init(1),mu_m1,Ncells_init]); % 'a': alpha(t,i) = a
...
[log_alpha_dispersion_timeindependent_inputindependent, LSE_percent_celldeath_timeindependent_inputindependent] = fminsearch(@(x)getfit_deathrate_dispersion_fixed(x,dispersion_m1,moi_vector_3hpi,moi_vector,moi_vector_FACs_data,counts_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timeindependent_inputindependent);
...
var_log_alpha_m1 = get_var_negbinomial(log_alpha_dispersion_timeindependent_inputindependent, moi_vector_3hpi, moi_vector, moi_vector_FACs_data, data.hpi, counts_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num, cell_line);
alpha_m1 = exp(log_alpha_dispersion_timeindependent_inputindependent');
% dispersion_m1 = alpha_m1(end);
...
% disp(['r (dispersion) = ',num2str(dispersion_m1,'%2.4f'), ', CI[', num2str(CI_m1(4,1),'%2.4f'),', ',num2str(CI_m1(4,2),'%2.4f'), '] ']);
disp(['Least squares error = ',num2str(LSE_percent_celldeath_timeindependent_inputindependent,'%2.2e')]);  fprintf('\n\n');
...
disp(['time-dependent, input-independent cell death rate: model ',num2str(model_num),' (Negative binomial)']); fprintf('\n');

% constant death rate:
mu_m2 = 0.02;

% model 2: dispersion parameter:
dispersion_m2 = r_dispersion_fixed;

% initialize minimization - shape and scale parameters of the Weibull hazard function
x0_timedependent_inputindependent = log([alpha_init(2:3),mu_m2,Ncells_init]); % 'k','b': alpha(t,i) = b*k*t^(k-1)
...
[log_alpha_dispersion_timedependent_inputindependent, LSE_percent_celldeath_timedependent_inputindependent] = fminsearch(@(x)getfit_deathrate_dispersion_fixed(x,dispersion_m2,moi_vector_3hpi,moi_vector,moi_vector_FACs_data,counts_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timedependent_inputindependent);

AIC_model2 = n_pts_allhpi_FACs*log(LSE_percent_celldeath_timedependent_inputindependent)+2*k_timedependent_inputindependent;

var_log_alpha_m2 = get_var_negbinomial(log_alpha_dispersion_timedependent_inputindependent, moi_vector_3hpi, moi_vector, moi_vector_FACs_data, data.hpi, counts_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num, cell_line);
alpha_m2 = exp(log_alpha_dispersion_timedependent_inputindependent');
% dispersion_m2 = alpha_m2(end);
...
% disp(['r (dispersion) = ',num2str(dispersion_m2,'%2.4f'), ', CI[', num2str(CI_m2(5,1),'%2.4f'),', ',num2str(CI_m2(5,2),'%2.4f'), '] ']);
disp(['Least squares error = ',num2str(LSE_percent_celldeath_timedependent_inputindependent,'%2.2e')]);  fprintf('\n\n');
...
disp(['time-independent, input-dependent cell death rate: model ',num2str(model_num),' (Negative binomial)']); fprintf('\n');

% constant death rate:
mu_m3 = 0.02;

% model 3: dispersion parameter:
dispersion_m3 = r_dispersion_fixed;

% initialize minimization - per capita death rate
x0_timeindependent_inputdependent = log([alpha_init(4:5),mu_m3,Ncells_init]); % 'c': alpha(t,i) = c*i^eps
...
[log_alpha_dispersion_timeindependent_inputdependent, LSE_percent_celldeath_timeindependent_inputdependent] = fminsearch(@(x)getfit_deathrate_dispersion_fixed(x,dispersion_m3,moi_vector_3hpi,moi_vector,moi_vector_FACs_data,counts_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timeindependent_inputdependent);

AIC_model3 = n_pts_allhpi_FACs*log(LSE_percent_celldeath_timeindependent_inputdependent)+2*k_timeindependent_inputdependent;

var_log_alpha_m3 = get_var_negbinomial(log_alpha_dispersion_timeindependent_inputdependent, moi_vector_3hpi, moi_vector, moi_vector_FACs_data, data.hpi, counts_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num, cell_line);
alpha_m3 = exp(log_alpha_dispersion_timeindependent_inputdependent');
% dispersion_m3 = alpha_m3(end);
...
% disp(['r (dispersion) = ',num2str(dispersion_m3,'%2.4f'), ', CI[', num2str(CI_m3(5,1),'%2.4f'),', ',num2str(CI_m3(5,2),'%2.4f'), '] ']);
disp(['Least squares error = ',num2str(LSE_percent_celldeath_timeindependent_inputdependent,'%2.2e')]);  fprintf('\n\n');
...
disp(['time-dependent, input-dependent cell death rate: model ',num2str(model_num),' (Negative binomial)']); fprintf('\n');

% constant death rate:
mu_m4 = 0.02;

% model 4: dispersion parameter:
dispersion_m4 = r_dispersion_fixed;

% minimize sum squared error to get estimates of 'k' and 'd'
% initialize minimization - per capita death rate
x0_timedependent_inputdependent = log([alpha_init(6:8),mu_m4,Ncells_init]); % 'k','d': alpha(t,i) = b4*k4*t^(k4-1)*i^eps
...
[log_alpha_dispersion_timedependent_inputdependent, LSE_percent_celldeath_timedependent_inputdependent] = fminsearch(@(x)getfit_deathrate_dispersion_fixed(x,dispersion_m4,moi_vector_3hpi,moi_vector,moi_vector_FACs_data,counts_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timedependent_inputdependent);

AIC_model4 = n_pts_allhpi_FACs*log(LSE_percent_celldeath_timedependent_inputdependent)+2*k_timedependent_inputdependent;

var_log_alpha_m4 = get_var_negbinomial(log_alpha_dispersion_timedependent_inputdependent, moi_vector_3hpi, moi_vector, moi_vector_FACs_data, data.hpi, counts_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num, cell_line);
alpha_m4 = exp(log_alpha_dispersion_timedependent_inputdependent');
% dispersion_m4 = alpha_m4(end);
...
% disp(['r (dispersion) = ',num2str(dispersion_m4,'%2.4f'), ', CI[', num2str(CI_m4(6,1),'%2.4f'),', ',num2str(CI_m4(6,2),'%2.4f'), '] ']);
disp(['Least squares error = ',num2str(LSE_percent_celldeath_timedependent_inputdependent,'%2.2e')]);  fprintf('\n\n');

% overwrite dispersion parameter
% dispersion_ovewrite = 0.2;
% dispersion_m1 = dispersion_ovewrite;
% dispersion_m2 = dispersion_ovewrite;
% dispersion_m3 = dispersion_ovewrite;
% dispersion_m4 = dispersion_ovewrite;
% pause;
% ---------------------------------- % ---------------------------------- %
...
for k = 1:length(data.hpi)
    ...
    m1_cellsremaining = (alpha_m1(end)/100)*get_percentalive(alpha_m1,data.hpi(k),moi,dispersion_m1,1,which_distribution); % last two: which model, which distribution
    m2_cellsremaining = (alpha_m2(end)/100)*get_percentalive(alpha_m2,data.hpi(k),moi,dispersion_m2,2,which_distribution);
    m3_cellsremaining = (alpha_m3(end)/100)*get_percentalive(alpha_m3,data.hpi(k),moi,dispersion_m3,3,which_distribution);
    m4_cellsremaining = (alpha_m4(end)/100)*get_percentalive(alpha_m4,data.hpi(k),moi,dispersion_m4,4,which_distribution);
...
%         r(1) = semilogy(moi,m1_cellsremaining,'--','Color',rgb_values(k,:),'LineWidth',1); hold on;
%         r(2) = semilogy(moi,m2_cellsremaining,'Color',rgb_values(k,:),'LineWidth',1); hold on;
%         r(3) = semilogy(moi,m3_cellsremaining,'--','Color',rgb_values(k,:),'LineWidth',3); hold on;
%         r(4) = semilogy(moi,m4_cellsremaining,'Color',rgb_values(k,:),'LineWidth',3); hold on;
...
%         semilogy(moi,m1_cellsremaining,'--','Color',rgb_values(k,:),'LineWidth',1); hold on;
%         semilogy(moi,m2_cellsremaining,'Color',rgb_values(k,:),'LineWidth',1); hold on;
%         semilogy(moi,m3_cellsremaining,'--','Color',rgb_values(k,:),'LineWidth',3); hold on;
%         semilogy(moi,m4_cellsremaining,'Color',rgb_values(k,:),'LineWidth',3); hold on;
    end
    ...
    if k == 4 % 18 hpi time point
        ...
        m1_percentremaining_infected_nb = get_percentaliveinfected(alpha_m1,data.hpi(k),moi,dispersion_m1,1,which_distribution); % which model, which distirbution
        m2_percentremaining_infected_nb = get_percentaliveinfected(alpha_m2,data.hpi(k),moi,dispersion_m2,2,which_distribution);
        m3_percentremaining_infected_nb = get_percentaliveinfected(alpha_m3,data.hpi(k),moi,dispersion_m3,3,which_distribution);
        m4_percentremaining_infected_nb = get_percentaliveinfected(alpha_m4,data.hpi(k),moi,dispersion_m4,4,which_distribution);
...
%         subplot(3,1,2);
        ...
        q(3) = plot(moi,m1_percentremaining_infected_nb,'k--','LineWidth',1); hold on;
        q(4) = plot(moi,m2_percentremaining_infected_nb,'k','LineWidth',1); hold on;
        q(5) = plot(moi,m3_percentremaining_infected_nb,'k--','LineWidth',3); hold on;
        q(6) = plot(moi,m4_percentremaining_infected_nb,'k','LineWidth',3); hold on;
        axis([0 10 0 100]);
        xlabel('Bulk MOI'); ylabel('Percent of alive cells that are infected');
        title([fig_title,': Negative binomial']);
        ax = gca;
        ax.FontSize = 10;
        ax.FontWeight = 'bold';
        ax.FontName = 'Arial';
...
figure(3);
sp = suptitle([fig_title,': Negative binomial']);
sp.FontSize = 12;
sp.FontWeight = 'Bold';
...
ax.FontName = 'Arial';
% legend(p,{'time-independent, input-independent','time-dependent, input-independent','time-independent, input-dependent'},'Location','NorthEast');
% legend(q,{'time-independent, input-independent','time-dependent, input-independent','time-independent, input-dependent','time-dependent, input-dependent'},'Location','NorthEast');
...
results.uninfectedcell_deathrate = [uninfectedcell_deathrate_m1',uninfectedcell_deathrate_m2',uninfectedcell_deathrate_m3',uninfectedcell_deathrate_m4'];
...
results.get_percentalive_infected = [m1_percentremaining_infected_nb',m2_percentremaining_infected_nb',m3_percentremaining_infected_nb',m4_percentremaining_infected_nb'];
