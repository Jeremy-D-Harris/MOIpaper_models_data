function sumsq_error = getfit_deathrate_dispersion(log_alpha_dispersion, moi_vector_3hpi, moi_vector, moi_vector_FACs_data, data_allhpi, FACs_data_18hpi, hpi, model_num, which_distribution)
...
switch model_num
    ...
    case 1
        ...
        celldeath_rate_params = alpha_dispersion(1:2); % includes mu
        Ncells_init = alpha_dispersion(3);
%         mu = alpha_dispersion(2); % constant death rate of uninfected cells
        r_dispersion = alpha_dispersion(end);
        
    case 2
        % 'model 2': Weibull hazard function cell death rate, input-independent
        celldeath_rate_params = alpha_dispersion(1:3); % includes mu
        Ncells_init = alpha_dispersion(4);
...
%         mu = alpha_dispersion(3); % constant death rate of uninfected cells
    ...
    case 4
        ...
        celldeath_rate_params = alpha_dispersion(1:4); % includes mu
        Ncells_init = alpha_dispersion(5);
...
for n = 1:length(hpi)
    ...
    if n == 4 % 18 hpi
        ...
        percentcells_alive = get_percentalive(celldeath_rate_params,this_hpi,these_moi,r_dispersion,model_num,which_distribution);
...
%         sumsq_error = sumsq_error + sum((log10(data_this_hpi) - log10(percentcells_alive)).^2); % log
        ...
        percent_aliveinfected = get_percentaliveinfected(celldeath_rate_params,this_hpi,these_moi_FACs,r_dispersion,model_num,which_distribution);
