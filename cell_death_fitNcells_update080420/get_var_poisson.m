function sample_param_var_estimate = get_var_poisson(log_alpha, moi_vector_3hpi, moi_vector, moi_vector_FACs_data, all_hpi, data_allhpi, FACs_data_18hpi, model_num)
...
which_distribution = 1;


alpha = exp(log_alpha);
Ncells_init = alpha(end);
...
for n = 1:length(all_hpi)
    ...
    % at 18 hpi, include FACs data also
    if n ==4
        
        % percent cells remaining
        for k = 1:length(these_moi_vals)
            ...
            for m = 1:num_pars
                ...
                if m == num_pars % different for N cells init
        ...
        % percent cells infected - FACs data
    ...
    % all other hours post infection: 3,6,12 hpi
    else
