function results = submain_celldeath_negbinomial(data,FACs_data_18hpi_vector,alpha_init,Ncells_init,r_dispersion_init,cell_line)
...
r_dispersion_m1 = r_dispersion_init(1);

% initialize minimization - cell death rate
x0_timeindependent_inputindependent = log([alpha_init(1),mu_m1,Ncells_init,r_dispersion_m1]); % 'a': alpha(t,i) = a
...
r_dispersion_m2 = r_dispersion_init(2);

% initialize minimization - shape and scale parameters of the Weibull hazard function
x0_timedependent_inputindependent = log([alpha_init(2:3),mu_m2,Ncells_init,r_dispersion_m2]); % 'k','b': alpha(t,i) = b*k*t^(k-1)
...
r_dispersion_m3 = r_dispersion_init(3);

% initialize minimization - per capita death rate
x0_timeindependent_inputdependent = log([alpha_init(4:5),mu_m3,Ncells_init,r_dispersion_m3]); % 'c': alpha(t,i) = c*i^eps
...
r_dispersion_m4 = r_dispersion_init(4);

% minimize sum squared error to get estimates of 'k' and 'd'
% initialize minimization - per capita death rate
x0_timedependent_inputdependent = log([alpha_init(6:8),mu_m4,Ncells_init,r_dispersion_m4]); % 'k','d': alpha(t,i) = b4*k4*t^(k4-1)*i^eps
...
Ncell_init_m1 = alpha_m1(end-1);
Ncell_init_m2 = alpha_m2(end-1);
Ncell_init_m3 = alpha_m3(end-1);
Ncell_init_m4 = alpha_m4(end-1);
...
for k = 1:length(data.hpi)
    ...
    m1_cellsremaining = (Ncell_init_m1/100)*get_percentalive(alpha_m1,data.hpi(k),moi,dispersion_m1,1,which_distribution); % last two: which model, which distribution
    m2_cellsremaining = (Ncell_init_m2/100)*get_percentalive(alpha_m2,data.hpi(k),moi,dispersion_m2,2,which_distribution);
    m3_cellsremaining = (Ncell_init_m3/100)*get_percentalive(alpha_m3,data.hpi(k),moi,dispersion_m3,3,which_distribution);
    m4_cellsremaining = (Ncell_init_m4/100)*get_percentalive(alpha_m4,data.hpi(k),moi,dispersion_m4,4,which_distribution);
